...
namespace ATmega {
  ...
  {
#ifdef DEBUG_EE_PRM_DB
    DEBUG_Serial.print(F("Get port call was made, looking for ID: 0x"));
    ...
    for (NATIVE_INT_TYPE entry = 0; entry < m_totalSlotCount; entry++) {
      if (isUsed(entry)) {
...
#endif
          for(NATIVE_UINT_TYPE i; i<m_prmBufCache.entry.length; i++)
          {
            buf[i] = m_prmBufCache.entry.buffer[i];
#ifdef DEBUG_EE_PRM_DB
            DEBUG_Serial.print(buf[i]);
#endif
          }
        ...
        {
          DEBUG_Serial.print(F("entry ID was: 0x"));
    ...
    )
  {
    ...
    DEBUG_Serial.print(F("Param ID: 0x"));
